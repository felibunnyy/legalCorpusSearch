...
# python3 search2.py -d dictionary_file14 -p postings_file14 -q q1.txt -o output-file-of-results
...
def find_results(query, in_memory_dictionary, posting_list_file, raw_doc_length_dict):
        ...
        if(token in in_memory_dictionary):
            ...
            if (token in curr_query_term_freq_mapping):
                ...
                for curr_pointer in range(len(posting_list)):
                    ...
                    #DEBUGSTART
                    if (currDocId == 2762682):
                      print('2762682, curr term:', token, "tf: ", curr[1])
                    if (currDocId == 3992148):
                      print('3992148, curr term:', token, "tf: ", curr[1])
                    #DEBUGEND
                    #artificially added "/math.sqrt(raw_doc_length_dict[currDocId]" part to "try to normalise" scores
                    cosine_without_normalisation[currDocId][mappingid] = (1 + math.log10(curr[1]))
    #DEBUGSTART
    # print("2762682", cosine_without_normalisation[2762682])
    # print("3992148", cosine_without_normalisation[3992148])
    #DEBUGEND
...
def helperSearch(query, in_memory_dictionary, posting_list_file, final_calculated_normalised_length, court_mapping, court_score_mapping, zones_and_fields_dict, N, raw_doc_length_dict):
    ...
    curr_query_term_freq, cosine_without_normalisation, curr_query_term_freq_mapping = find_results(query_after_processing, in_memory_dictionary, posting_list_file, raw_doc_length_dict)
    # print("curr_query_term_freq")
    # print(curr_query_term_freq)
    ...
    modifyScoreByTitle_Court_Date(score_dict, query, court_mapping, court_score_mapping, zones_and_fields_dict, raw_doc_length_dict)
...
def run_search(dictionary_file, postings_file, query_file, results_file):
    ...
    with open(dictionary_file, 'rb') as f:
        ...
        raw_doc_length_dict = pickle.load(f)
    ...
    with open(query_file, 'r') as file:
        ...
        new_query = ''
        if "AND" in query:
            queries = query.split("AND") #split is essentially not effective here rel
            for each_query in queries:
                print("ehflkjs", each_query)
                print(each_query[0], each_query[-1])
                if (each_query[0] == each_query[-1] and each_query[0] == '"' or each_query[0] == "'"):
                  print("true", each_query)
        ...
        if (len(relevant_docs) == 0):
            query = query_expansion(query, in_memory_dictionary, N, posting_list_file)
        score_dict, curr_query_term_freq, curr_query_term_freq_mapping = helperSearch(query, in_memory_dictionary, posting_list_file, final_calculated_normalised_length, court_mapping, court_score_mapping, zones_and_fields_dict, N, raw_doc_length_dict)

    
        ###perform relevance queryid_to
        if (len(relevant_docs) > 0):
            reformulated_query = perform_relevance_query(curr_query_term_freq, curr_query_term_freq_mapping, relevant_docs, in_memory_dictionary, relevance_query_dict, N, raw_doc_length_dict)
            if (reformulated_query != False):
                score_dict, curr_query_term_freq, curr_query_term_freq_mapping = helperSearch(reformulated_query, in_memory_dictionary, posting_list_file, final_calculated_normalised_length, court_mapping, court_score_mapping, zones_and_fields_dict, N, raw_doc_length_dict)
        
        
        #####perform tweaking of results by perform_relevance_query (MODIFY SCORE_DICT)
        # artificially added "/math.sqrt(raw_doc_length_dict[currDocId])" part to "try to normalise" scores
        for currDocId, currScore in score_dict.items():
          # normalising_factor = max(1, (math.sqrt(raw_doc_length_dict[currDocId])))
          normalising_factor = max(1, (math.log10(raw_doc_length_dict[currDocId]) * math.sqrt(raw_doc_length_dict[currDocId])))
          # if (currDocId == 6807771):
          #   print("6807771", normalising_factor)
          # elif (currDocId == 4001247):
          #   print("4001247", normalising_factor)
          # elif (currDocId == 3992148):
          #   print("3992148", normalising_factor)
          
          # if (currDocId == 2211154):
          #   print("2211154", normalising_factor)
          # elif (currDocId == 2748529):
          #   print("2748529", normalising_factor)
            
          # if (currDocId == 4273155):
          #   print("4273155", normalising_factor)
          # elif (currDocId == 3243674):
          #   print("3243674", normalising_factor)
          # elif (currDocId == 2702938):
          #   print("2702938", normalising_factor)
          newScore = currScore / normalising_factor
          score_dict[currDocId] = newScore
        # print("6807771", score_dict[6807771])
        # print("4001247", score_dict[4001247])
        # print("3992148", score_dict[3992148])
        
        # print("2211154", score_dict[2211154])
        # print("2748529", score_dict[2748529])
        
        # print("4273155", score_dict[4273155])
        # print("3243674", score_dict[3243674])
        # print("2702938", score_dict[2702938])
...
###add query expansion
def query_expansion(raw_query, in_memory_dictionary, N, posting_list_file):
    print("expand")
    ...
    for token in tokens:
        ...
        # print(token)
        if processed_token in in_memory_dictionary:
            doc_freq = in_memory_dictionary[processed_token][0]
...
def perform_relevance_query(curr_query_term_freq, curr_query_term_freq_mapping, relevant_docs, in_memory_dictionary, relevance_query_dict, N,raw_doc_length_dict):
...
def modifyScoreByTitle_Court_Date(score_dict, query, court_mapping, court_score_mapping, zones_and_fields_dict, raw_doc_length_dict):
    bonus_date_score = 0.015
    for docId, currScore in score_dict.items():
        #####perform tweaking of results by perform_relevance_query (MODIFY SCORE_DICT)
        #artificially added "/math.sqrt(raw_doc_length_dict[currDocId])" part to "try to normalise" scores
        # normalising_factor = math.sqrt(raw_doc_length_dict[docId])
        # newScore = currScore / normalising_factor
        # score_dict[docId] = newScore
...
def sort_score_dict_by_score_descending(score_dict):
    ...
    for item in sorted_result:
        # TESTING_COUNTER += 1
        # if (item[0] == 6807771):
        #   print("6807771", TESTING_COUNTER)
        # elif (item[0] == 4001247):
        #   print("4001247", TESTING_COUNTER)
        # elif (item[0] == 3992148):
        #   print("3992148", TESTING_COUNTER)
        
        
        # if (item[0] == 2211154):
        #   print("2211154", TESTING_COUNTER)
        # elif (item[0] == 2748529):
        #   print("2748529", TESTING_COUNTER)
        
        # if (item[0] == 4273155):
        #   print("4273155", TESTING_COUNTER)
        # elif (item[0] == 3243674):
        #   print("3243674", TESTING_COUNTER)
        # elif (item[0] == 2702938):
        #   print("2702938", TESTING_COUNTER)
